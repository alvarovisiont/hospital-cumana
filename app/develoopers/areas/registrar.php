<? 
	...
	if(isset($_GET['modificar']))
	{
		...
		$area = $system->find($_GET['modificar']);
	}
?>
	<form action="#" class="form-horizontal" id="form_registrar">
		<h3 class="text-center"><span class="subrayado_rojo">Registro de Areas</span></h3>
		<br>
		<input type="hidden" name="action" value="<?= $area ? 'modificar' : 'registrar'; ?>">
		<input type="hidden" name="id_modificar" value="<?= $area ? $area->id : ''; ?>">
		<input type="hidden" name="created_at" value="<?= $area ? $area->created_at : date('Y-m-d H:i:s', strtotime('-6 hour')); ?>">
		...
		<div class="form-group">
				...
				<select name="departamento_id" id="departamento_id" class="form-control" required="">
					...
					<?
						...
						foreach ($system->sql() as $row) 
						{
							if($area)
							{
								if($area->departamento_id == $row->id)
								{
			...
			<div class="col-md-4">
				<input type="text" id="nombre" name="nombre" class="form-control" required="" value="<?= $area ? $area->nombre : ''; ?>">
			</div>
		</div>
		<div class="form-group">
			...
			<div class="col-md-offset-1 col-md-3">
				<a href="<?= './index.php' ?>">Regresar a la Vista de Areas</a>
			</div>
		</div>
